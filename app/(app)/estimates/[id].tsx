...
export default function EstimateDetailScreen() {
  ...
  const handleDelete = () => {
    const doIt = async () => {
      setDeleting(true);
      try {
        await estimatesApi.remove(parseInt(id!));
        router.back();
      } catch (e: any) {
        const msg = e?.response?.data?.error ?? 'Could not delete estimate.';
        setDeleting(false);
        if (Platform.OS === 'web') window.alert(msg); else Alert.alert('Cannot Delete', msg);
      }
    };
    const label = estimate?.estimateNo ?? 'this estimate';
    if (Platform.OS === 'web') {
      if (window.confirm(`Permanently delete "${label}"? This cannot be undone.`)) doIt();
      return;
    }
    Alert.alert('Delete Estimate', `Permanently delete "${label}"? This cannot be undone.`, [
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.totalBar}>
          {estimate.discountEnabled && parseFloat(estimate.discountPercent) > 0 ? (
            <View style={{ flex: 1 }}>
              <View style={styles.totalSubRow}>
                <Text style={styles.totalSubLabel}>Subtotal</Text>
                <Text style={styles.totalSubValue}>{fmtAmt(estimate.subtotal)}</Text>
              </View>
              <View style={styles.totalSubRow}>
                <Text style={styles.totalSubLabel}>Discount ({parseFloat(estimate.discountPercent)}%)</Text>
                <Text style={[styles.totalSubValue, { color: Colors.error }]}>
                  −{fmtAmt(String(parseFloat(estimate.subtotal) - parseFloat(estimate.totalAmount)))}
                </Text>
              </View>
              <View style={[styles.totalSubRow, { marginTop: 4 }]}>
      ...
      <EstimateFormSheet
        ...
        onSaved={() => { setShowEdit(false); loadEstimate(); }}
      />
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  totalSubRow:   { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 2 },
  totalSubLabel: { fontSize: 12, color: Colors.textMuted },
  totalSubValue: { fontSize: 12, fontWeight: '600', color: Colors.textSecondary },
